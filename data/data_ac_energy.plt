...
do for [fn in list_data] {
	...
	a = 1
	b = -1
	fit [0:1600] f(x) fn using 1:2 via a,b
	TITLE_FIT=sprintf("f(x) = %f * exp(%f * x)", a, b)
	plot [0:200] fn u 1:2 w l lw 2 title TITLE, f(x) lw 2 title TITLE_FIT
	}
unset multiplot
unset output
